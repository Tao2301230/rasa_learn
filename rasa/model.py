...
class FingerprintComparisonResult:
    def __init__(self, nlu: bool = True, core: bool = True, nlg: bool = True, force_training: bool = False, ):
...
def get_model_subdirectories(unpacked_model_path: Text, ) -> Tuple[Optional[Text], Optional[Text]]:
    ...
    if not core_path and not nlu_path:
        raise ModelNotFound("No NLU or Core data for unpacked model at: '{}'.".format(unpacked_model_path))

    return core_path, nlu_path


def create_package_rasa(training_directory: Text, output_filename: Text, fingerprint: Optional[Fingerprint] = None,
                        ) -> Text:
...
async def model_fingerprint(file_importer: "TrainingDataImporter") -> Fingerprint:
    ...
    return {
        ...
        FINGERPRINT_CONFIG_CORE_KEY: _get_hash_of_config(config, include_keys=CONFIG_KEYS_CORE),
        FINGERPRINT_CONFIG_NLU_KEY: _get_hash_of_config(config, include_keys=CONFIG_KEYS_NLU),
...
def _get_hash_of_config(config: Optional[Dict], include_keys: Optional[List[Text]] = None,
                        exclude_keys: Optional[List[Text]] = None, ) -> Text:
    if not config:
        return ""
...
def should_retrain(new_fingerprint: Fingerprint, old_model: Text, train_path: Text) -> FingerprintComparisonResult:
    ...
    with unpack_model(old_model) as unpacked:
        ...
        fingerprint_comparison = FingerprintComparisonResult(
            core=did_section_fingerprint_change(last_fingerprint, new_fingerprint, SECTION_CORE),
            nlu=did_section_fingerprint_change(last_fingerprint, new_fingerprint, SECTION_NLU),
            nlg=did_section_fingerprint_change(last_fingerprint, new_fingerprint, SECTION_NLG),
        )
...
def package_model(fingerprint: Fingerprint, output_directory: Text, train_path: Text,
                  fixed_model_name: Optional[Text] = None, model_prefix: Text = "", ):
    """
    ...
    """
    output_directory = create_output_path(output_directory, prefix=model_prefix, fixed_name=fixed_model_name)
    create_package_rasa(train_path, output_directory, fingerprint)

    print_success("Your Rasa model is trained and saved at '{}'.".format(os.path.abspath(output_directory)))

    return output_directory


async def update_model_with_new_domain(importer: "TrainingDataImporter", unpacked_model_path: Union[Path, Text]) -> None:
