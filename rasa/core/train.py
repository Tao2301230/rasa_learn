...
async def train(
    ...
    agent = Agent(domain_file, generator=endpoints.nlg, action_endpoint=endpoints.action, interpreter=interpreter, policies=policies, )
    ...
    training_data = await agent.load_data(training_resource, exclusion_percentage=exclusion_percentage, **data_load_args
    )
    ...
    for r in range(runs):
        ...
        for current_run, percentage in enumerate(exclusion_percentages, 1):
            for policy_config in policy_configs:
                ...
                file_importer = TrainingDataImporter.load_core_importer_from_config(policy_config, domain, [story_file])
